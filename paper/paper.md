...
authors:
  - name: Arnoneel Sinha
    orcid: 0000-0003-1006-0683 # TODO: replace with Arnoneel Sinha's ORCID iD
    affiliation: 1
  - name: Kathryn Sinha
    orcid: 0000-0000-0000-0000 # TODO: provide ORCID iD (to be supplied later)
    affiliation: 2
...
Rather than re-implementing single-cell algorithms, `cellivista` deliberately *builds upon* the Seurat ecosystem and exposes it through a browser interface. Its distinct contribution is to deliver the *complete* exploratory Seurat workflow — including steps that hosted portals typically leave to the user's own scripts, such as doublet removal and SCT-based integration — in a single, no-code, locally runnable application, while keeping the underlying Seurat objects exportable at every stage. `cellivista` can be used either through a hosted deployment or by running the application locally from its source repository. Its computational workflow is implemented as modular helper functions that are separate from the reactive Shiny interface, allowing individual analytical steps to be inspected, tested, and reused independently while preserving the interactive user experience. 
